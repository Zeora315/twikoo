...
function renderConsent() {
  ...
  els.userUid.textContent = `HeoID: ${user.uid || String(user.id || '').slice(0, 8)}`;
  setAvatar(user);
...
document.querySelector('[data-close]').addEventListener('click', cancelAuthorization);
...
(async function boot() {
  document.title = `HeoID 授权登录 - ${state.siteName}`;
